...
const SubscriptionSchema =  z.object({
    ...
    price: z.coerce.number().min(0).transform(value => (Number(value))),
...
const ListSubscriptions = async (): Promise<Subscription[]> => {
    const {data, error} = await Supabase.from(SUBSCRIPTION_TABLE).select(`
          id,
          name,
          payment_every,
          payment_frequency,
    ...
    `).order('created_at', {ascending: false}).returns<Subscription[]>();
